...
description: |
  ...
  不要用于简单的名词解释（用户只是问"XX是什么"）、不要用于公众号写作（那个用longform-writer）、不要用于纯标题摘要生成。
---
...
这不是一份冷冰冰的咨询报告，而是一篇让人能从头读到尾的深度研究。写作风格需要在「研究报告的严谨」和「长文可读性」之间找到平衡点。
...
- **封面页**：自动生成，包含标题（28pt深蓝色）、副标题「横纵分析法深度研究报告」、可选作者信息、装饰分隔线
