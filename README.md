...
这里收录了一组可直接复用的 AI Skill，覆盖磁盘清理、AI 资讯查询、知识同步、深度研究和公众号长文写作等场景。
...
| 名字 | 一句话 | 说明 |
|---|---|---|
| 💽 [**storage-analyzer（清理垃圾）**](#-storage-analyzer清理垃圾) | 一句话扫描 Mac / Windows 整机磁盘，三色分级给清理决策，网页上一键移废纸篓 | [SKILL.md](./storage-analyzer/SKILL.md) |
...
| 🧹 [**neat-freak（洁癖）**](#-neat-freak洁癖) | 干完活跑一下 `/neat`，自动对齐项目文档、CLAUDE.md、Agent 记忆，并审计规则有没有被执行 | [SKILL.md](./neat-freak/SKILL.md) |
| 🔭 [**hv-analysis（横纵分析法）**](#-hv-analysis横纵分析法) | 想搞懂一个产品/公司/概念是怎么回事，丢给它，给你一份万字 PDF 研究报告 | [SKILL.md](./hv-analysis/SKILL.md) |
| ✍️ [**longform-writer（公众号长文写作）**](#-longform-writer公众号长文写作) | 让 Agent 按一套固定口吻、节奏和禁忌词规则写公众号长文 | [SKILL.md](./longform-writer/SKILL.md) |
...
```
帮我安装这个 skill：https://github.com/forest-USTC/ai-skills/tree/main/<skill-name>
```

把 `<skill-name>` 换成你想装的那个，比如 `neat-freak`、`hv-analysis`、`longform-writer`。Agent 会自己 clone 到对应目录，不用你操心路径。
...
### ✍️ longform-writer（公众号长文写作）

> *"有见识的普通人在认真聊一件打动他的事。"*

一套面向中文公众号长文的写作 skill。装上之后，Agent 会按固定口吻、节奏和禁忌词规则生成长文。

**适合**

你需要把一篇 PDF / 一段语音转文字 / 一个新闻链接 / 一组素材整理成中文公众号长文，并希望输出保持稳定的口语化风格。
...
本仓库收录的 Skill 可按需复制、修改和再分发。使用前建议先阅读每个目录下的 `SKILL.md`，确认触发场景、依赖和限制。
